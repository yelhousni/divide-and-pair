...
const (
	Limbs = 2   // number of 64 bits words needed to represent a Element
	Bits  = 127 // number of bits needed to represent a Element
	Bytes = 16  // number of bytes needed to represent a Element
)

// Field modulus q
const (
	q0 = 18446744073709551615
	q1 = 9223372036854775807
)
...
func (z *Element) SetInterface(i1 any) (*Element, error) {
	if i1 == nil {
		return nil, errors.New("can't set fp127.Element with <nil>")
	}
	...
	default:
		return nil, errors.New("can't set fp127.Element from type " + reflect.TypeOf(i1).String())
	}
}
...
func (z *Element) SetBytesCanonical(e []byte) error {
	if len(e) != Bytes {
		return errors.New("invalid fp127.Element encoding")
	}
	v, err := BigEndian.Element((*[Bytes]byte)(e))
	if err != nil {
		return err
...
var errInvalidEncoding = errors.New("invalid fp127.Element encoding")
...
func init() {
	...
	const sqrtExponent2Element = "1fffffffffffffffffffffffffffffff"
...
func (z *Element) Legendre() int {
	...
	for !a.IsZero() {
		...
		a[1] = (a[1] >> nbIterations) | (aHi << (2*k - nbIterations))
		...
		b[1] = (b[1] >> nbIterations) | (bHi << (2*k - nbIterations))
	}
...
func init() {
	_bCbrtExponentElement, _ = new(big.Int).SetString("1948b0fcd6e9e06522c3f35ba781949", 16)
}
...
func (z *Element) Cbrt(x *Element) *Element {
	...
	// e ≥ 3: Tonelli-Shanks variant
	var y, t, w, c Element
	// s ≡ 2 (mod 3), using (s+1)/3
	// y = x^((s+1)/3) is the initial candidate
	y.Exp(*x, _bCbrtExponentElement)
